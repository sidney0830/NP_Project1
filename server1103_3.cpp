...
char * parse(int fd,int line_sep_count,char*line[MAXLINE][MAXCMD])
{
    ...
    for(int j=0;j<line_sep_count;j++)
    {
            ...
            for(int i=0; line[j][i]!=NULL ;i++)
            {
                if (line[j][i][0]=='|')///
                {
                    ...
                    { 
                        ...
                        if(temp_write<0) pipe_array[now_pipe_count+temp][1]= now_pipe_count;
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if(temp_write<0) pipe_array[now_pipe_count+1][1]= now_pipe_count;
                        else
                        {
        ...
        else if (childpid > 0) //parent
        {
            ...
            if (readFromPipe)//
            {
                close(pipe_fd[now_pipe_count-1][0]);//減一是因為single pipe
            }
        }
        else if (childpid == 0)
        {
          ...
          // dup2 ( fd , 1); 
                ...
                if (readFromPipe) 
                {
                    ...
                    dup2(pipe_fd[now_pipe_count-1][0], STDIN_FILENO);//減一是因為single pipe
                    close(pipe_fd[now_pipe_count-1][0]);
                }
                if (writeToPipe) 
                {
             ...
             // fprintf (stderr,"--k\n"); // std::cout << "----in the fork----"  <<std::endl;
                ...
                if(execvp(line[j][0],line[j])== -1)
                {
                    ...
                    dup2(pipe_fd[now_pipe_count-1][1], STDOUT_FILENO);
                    // j=line_sep_count;
                    // std::cout << "Unknown command: [" << line[j][0] << "]" <<std::endl;
                    return 0;
                }
